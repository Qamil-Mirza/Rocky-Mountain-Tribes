...
export function NavBar({ transparent = false }: { transparent?: boolean }) {
  ...
  return (
    <div
      className={cn(
        "fixed top-0 left-0 right-0 z-50 px-4 py-2 transition-all duration-300",
        transparent && !scrolled ? "bg-transparent" : "bg-background dark:bg-backgroundDark shadow-sm",
        transparent && scrolled && "bg-background/90 backdrop-blur-sm dark:bg-backgroundDark/90",
      )}
    >
      <div className="container mx-auto flex h-16 items-center justify-between">
        <Link href="/" className="flex-shrink-0">
          <div className="flex items-center space-x-3">
            <Image src={logo || "/placeholder.svg"} alt="Logo" width={40} height={40} className="rounded-full" />
            <span
              className={cn(
                "text-lg font-bold",
                transparent && !scrolled ? "text-white" : "text-primaryText dark:text-white",
              )}
            >
              Rocky Mountains
            </span>
      ...
      <div
        className={cn(
          "fixed inset-0 top-[4rem] z-40 bg-background dark:bg-backgroundDark md:hidden transition-all duration-300 ease-in-out",
